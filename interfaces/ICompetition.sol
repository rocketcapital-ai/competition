...
interface ICompetition{
    ...
    **/
    ...
    event SubmissionUpdated(uint32 indexed challengeNumber,
        address indexed participantAddress, bytes32 indexed newSubmissionHash);
    ...
    event DatasetUpdated(uint32 indexed challengeNumber,
        bytes32 indexed oldDatasetHash, bytes32 indexed newDatasetHash);
    ...
    event ResultsUpdated(uint32 indexed challengeNumber,
        bytes32 indexed oldResultsHash, bytes32 indexed newResultsHash);

    event RewardsPayment(uint32 challengeNumber, address indexed submitter,
        uint256 stakingReward, uint256 indexed challengeReward, uint256 indexed tournamentReward);

    event TotalRewardsPaid(uint32 challengeNumber, uint256 indexed totalStakingAmount,
        uint256 indexed totalChallengeAmount, uint256 indexed totalTournamentAmount);
    ...
    **/
    ...
    /**
    * @dev Called only by authorized admin to update the minimum amount required in the competition rewards pool to
    ...
    **/
    ...
    /**
    * @dev Called only by authorized admin to update the private key for this challenge.
    This should be done at the end of the challenge.
    * @param challengeNumber Challenge to perform the update for.
    ...
    * @param datasetHash IPFS reference hash where dataset for this challenge is stored.
    This is the IPFS CID less the 1220 prefix.
    * @param keyHash IPFS reference hash where the key for this challenge is stored.
    This is the IPFS CID less the 1220 prefix.
    ...
    * @return success True if the operation completed successfully.
    **/
    function openChallenge(bytes32 datasetHash, bytes32 keyHash, uint256 submissionCloseDeadline,
        uint256 nextChallengeDeadline) external returns (bool success);

    /**
    * @dev Called only by authorized admin to update the IPFS reference for the dataset of a particular challenge.
    * @param newDatasetHash IPFS reference hash where new dataset for this challenge is stored.
    This is the IPFS CID less the 1220 prefix.
    * @return success True if the operation completed successfully.
    **/
    ...
    * @param newKeyHash IPFS reference hash where new key for this challenge is stored.
    This is the IPFS CID less the 1220 prefix.
    * @return success True if the operation completed successfully.
    **/
    ...
    * @param resultsHash IPFS reference hash where results for this challenge are stored.
    This is the IPFS CID less the 1220 prefix.
    * @return success True if the operation completed successfully.
    **/
    ...
    * @param oldResultsHash IPFS reference hash where previous results for this challenge are stored.
    This is the IPFS CID less the 1220 prefix.
    * @param newResultsHash IPFS reference hash where new results for this challenge are stored.
    This is the IPFS CID less the 1220 prefix.
    * @return success True if the operation completed successfully.
    **/
    ...
    /**
    * @dev Called only by authorized admin to move rewards from the competition pool to the winners' competition
    internal balances based on results from the current challenge.
    ...
    * @param stakingRewards List of corresponding amount of tokens in wei given to each submitter for
    the staking rewards portion.
    * @param challengeRewards List of corresponding amount of tokens in wei won by each submitter for
    the challenge rewards portion.
    * @param tournamentRewards List of corresponding amount of tokens in wei won by each submitter for
    the tournament rewards portion.
    ...
    **/
    function payRewards(address[] calldata submitters, uint256[] calldata stakingRewards,
        uint256[] calldata challengeRewards, uint256[] calldata tournamentRewards) external returns (bool success);
    ...
    **/
    function updateChallengeAndTournamentScores(uint32 challengeNumber, address[] calldata participants,
        uint256[] calldata challengeScores, uint256[] calldata tournamentScores) external returns (bool success);

    /**
    * @dev Called only by authorized admin to do a batch update of an additional information item
    for a list of participants for a given challenge.
    ...
    **/
    function updateInformationBatch(uint32 challengeNumber, address[] calldata participants,
        uint256 itemNumber, uint[] calldata values) external returns (bool success);
    ...
    * @dev In other words, payStakingRewards and payChallengeAndTournamentRewards may need to be called multiple times
    to complete all required payments.
    * @dev This function is used to advance to phase 3 after staking rewards payments have completed or to phase 4
    after challenge and tournament rewards payments have completed.
    ...
    **/
    ...
    /**
    * @dev Called only by an authorized admin, to move any tokens sent to this contract without using the
    'sponsor' or 'setStake'/'increaseStake' methods into the competition pool.
    * @return success True if the operation completed successfully.
    **/
    ...
    **/
    ...
    /**
    * @dev Called by a sponsor to send tokens to the contract's competition pool.
    This pool is used for payouts to challenge winners.
    * @dev This performs an ERC20 transfer so the msg sender will need to grant
    approval to this contract before calling this function.
    ...
    **/
    function getCompetitionPool() external view returns (uint256 competitionPool);

    /**
    * @dev Called by anyone to check minimum amount required to open a new challenge.
    * @return challengeRewardsThreshold Amount of tokens in wei the competition pool must contain to
    open a new challenge.
    **/
    function getRewardsThreshold() external view returns (uint256 challengeRewardsThreshold);
    ...
    **/
    function getCurrentTotalStaked() external view returns (uint256 currentTotalStaked);

    /**
    * @dev Called by anyone to get the number of the latest challenge.
    * @dev As the challenge number begins from 1, this is also the total number of challenges created in
    this competition.
    * @return latestChallengeNumber Latest challenge created.
    **/
    function getLatestChallengeNumber() external view returns (uint32 latestChallengeNumber);
    ...
    * @return dataset IPFS hash where the dataset of this particular challenge is stored.
    This is the IPFS CID less the 1220 prefix.
    **/
    function getDatasetHash(uint32 challengeNumber) external view returns (bytes32 dataset);
    ...
    * @return results IPFS hash where results of this particular challenge are stored.
    This is the IPFS CID less the 1220 prefix.
    **/
    function getResultsHash(uint32 challengeNumber) external view returns (bytes32 results);
    ...
    * @return key IPFS hash where results of this particular challenge are stored.
    This is the IPFS CID less the 1220 prefix.
    **/
    function getKeyHash(uint32 challengeNumber) external view returns (bytes32 key);

    /**
    * @dev Called by anyone to obtain the private key hash for this particular challenge.
    * @param challengeNumber The challenge to get the key hash of.
    * @return privateKey IPFS hash where results of this particular challenge are stored.
    This is the IPFS CID less the 1220 prefix.
    **/
    function getPrivateKeyHash(uint32 challengeNumber) external view returns (bytes32 privateKey);
    ...
    **/
    function getPhase(uint32 challengeNumber) external view returns (uint8 phase);
    ...
    **/
    function getStakeThreshold() external view returns (uint256 stakeThreshold);
    ...
    **/
    function getStake(address participant) external view returns (uint256 stake);
    ...
    * @return submissionHash IPFS reference hash of participant's prediction submission for this challenge.
    This is the IPFS CID less the 1220 prefix.
    **/
    function getSubmission(uint32 challengeNumber, address participant) external view returns (bytes32 submissionHash);
    ...
    **/
    ...
    external view returns (uint256 stakingRewards);
    ...
    **/
    ...
    external view returns (uint256 challengeRewards);
    ...
    **/
    ...
    external view returns (uint256 tournamentRewards);
    ...
    **/
    ...
    external view returns (uint256 challengeScores);
    ...
    **/
    ...
    external view returns (uint256 tournamentScores);
    ...
    **/
    ...
    /**
    * @dev Called by anyone to check the amount of tokens that have been sent to this contract
    but are not recorded as a stake or as part of the competition rewards pool.
    * @return remainder The amount of tokens held by this contract that are not recorded as a
    stake or as part of the competition rewards pool.
    **/
    ...
    **/
    function getSubmissionCounter(uint32 challengeNumber) external view returns (uint256 submissionCounter);
